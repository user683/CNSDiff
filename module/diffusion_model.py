...
class DiffusionProcess(nn.Module):
    ...
    def causal_regularization(self, emb_t, emb_t_prev, t):
        # Ensure input tensor dimensions are consistent
        assert emb_t.shape == emb_t_prev.shape

        logvar_t = self._extract_into_tensor(
            self.posterior_log_variance_clipped,
            t,
            (emb_t.shape[0], *([1] * (len(emb_t.shape) - 1)))
        ).expand_as(emb_t)
        ...
        logvar_t_prev = self._extract_into_tensor(
            self.posterior_log_variance_clipped,
            t - 1,
            (emb_t_prev.shape[0], *([1] * (len(emb_t_prev.shape) - 1)))
        ).expand_as(emb_t_prev)

        zero_tensor = th.zeros_like(emb_t_prev)  
        logvar_t_prev = th.where(
            (t == 0).view(-1, *([1] * (len(emb_t_prev.shape) - 1))), 
            zero_tensor,
            logvar_t_prev
        )
        ...
        kl_div = normal_kl(
            ...
            logvar1=logvar_t,
            mean2=emb_t_prev,
            logvar2=logvar_t_prev
        )
        return self.causal_weight * th.mean(kl_div)
    ...
    def caculate_losses(self, model, emb_s, reweight=False):
        ...
        terms["loss"] = weight * loss + causal_reg
    ...
    def p_sample(self, model, emb_s, steps, sampling_noise=False):
        ...
        if steps == 0:
            ...
            emb_t = self.q_sample(emb_s, t).float()  # Explicitly convert to float32
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        x_start: initial embedding vector
        t: timestep
        ...
        if noise is None:
            ...
            # Calculate cumulative noise ratio (alpha_bar)
        # Here we assume linear beta scheduling, can be modified as needed
    ...
    def q_posterior_mean_variance(self, emb_s, emb_t, t):
        ...
        # Use device uniformly for extraction and calculation
        posterior_mean = (
